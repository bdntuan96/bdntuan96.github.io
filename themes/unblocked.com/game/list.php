<?php
// check URL: remove characters => only get domain name: example: tuanlocal.com
...
// check if the directory path exists, if not, create it and set read/write permissions for it
...
if ($game) {
    ...
    $layout .= \helper\themes::get_layout('game_play', array('game' => $game, 'is_game_play_home' => true));
    $layout .= \helper\themes::get_layout('header/richtext_home', array('game' => $game));
    $layout .= \helper\themes::get_layout('footer');
    $layout = str_replace('://unblocked.com', "://$base_url", $layout);

    // check if the file is a normal file: if not Set permissions 0755: Owner can read, write and execute. Group and Others can read and execute
    ...
    echo '<div style="color: red;font-size: larger;font-weight: bold;">Tạo file Thất Bại index.php: do game trang chủ (slug_home) không tồn tại</div><br>';
}
